...
I dati strutturati (personaggi) vengono salvati in un database SQLite locale tramite **Room**. Il layer è così strutturato:
...
- `AppDatabase.kt` — singleton `RoomDatabase`, creato in `DmtApplication`.
...
- Le preferenze sono lette in `MainActivity` e passate a `DMTTheme`, così il tema è applicato globalmente prima che qualsiasi schermata venga composta.

## Tema

Il tema dell'app è controllato dall'enum `AppTheme` (`LIGHT`, `DARK`, `SYSTEM`). `DMTTheme` riceve il valore corrente da `MainActivity`, che lo osserva dal repository tramite `collectAsStateWithLifecycle`. `SYSTEM` delega a `isSystemInDarkTheme()`, gli altri due forzano la palette corrispondente indipendentemente dal sistema.
...
├── DmtApplication.kt              ← Application, espone db e repository
├── MainActivity.kt
├── model/
...
Ogni nuova schermata segue questo schema. Il ViewModel riceve il repository tramite factory (`viewModelFactory { initializer { ... } }`); la factory viene costruita in `AppNavigation` leggendo il repository da `DmtApplication`.
